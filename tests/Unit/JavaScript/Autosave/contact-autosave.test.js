...
import ContactAutosaveAdapter, { STRING_KEYS, validatePayload } from '../../../../media_source/com_contact/src/contact-autosave-adapter.es6.js';
...
const adapterFixture = () => {
  const source = {
    ...
    publish_up_alt: '2026-08-27 00:00:00', webpage: 'https://example.test',
  };
  ...
  const adapter = new ContactAutosaveAdapter({ descriptor: { context: 'com_contact.contact', targetId: '42', payloadSchemaVersion: 1 }, form, fields, editor, getCurrentEditor: () => editor, mediaField });
...
test('Contact captures the exact privacy-minimized contract including incomplete PII', () => {
  ...
  assert.deepEqual(adapter.capture(), source);
  assert.deepEqual(Object.keys(adapter.capture()), STRING_KEYS);
...
test('Contact restore uses editor, calendar, and media APIs without changing unrelated controls', () => {
  const { adapter, fields, source } = adapterFixture();
  const unrelated = new Control('jform_catid', '3');
  const restored = { ...source, email_to: '', telephone: '', misc: '<p>Restored</p>', image: '', publish_up: '', publish_up_alt: '' };
  ...
  assert.equal(unrelated.value, '3');
});

test('Contact dirty observation covers PII and editor while teardown is idempotent', () => {
...
test('Contact configuration enforces existing IDs and exact canonical actions', () => {
  const config = { enabled: true, context: 'com_contact.contact', targetId: '42', payloadSchemaVersion: 1, formId: 'contact-form', fieldIds };
...
test('eligible Contact activates one runtime and stale form teardown remains safe', async () => {
  ...
  const fields = Object.fromEntries(FIELD_KEYS.map((key) => [key, new Control(fieldIds[key], '')]));
  ...
  const runtimes = [];
  class Runtime { constructor(options) { this.options = options; this.destroyed = 0; } start() { this.unsubscribe = this.options.adapter.subscribe(() => {}); return this; } destroy() { if (!this.destroyed) { this.destroyed = 1; this.unsubscribe?.(); this.options.adapter.destroy(); } } }
  const controller = new ContactAutosaveController({
    documentSource, editorRegistry: { get: () => editor, subscribeLifecycle: () => () => {} },
    optionsReader: (key, fallback) => (key === OPTIONS_KEY ? { enabled: true, context: 'com_contact.contact', targetId: '42', payloadSchemaVersion: 1, formId: 'contact-form', fieldIds } : key === 'com_autosave.runtime' ? { endpoints: { initialize: 'i', preserve: 'p', detect: 'd', read: 'r', discard: 'x', prepareCanonicalAction: 'c', getCanonicalActionOutcome: 'o' } } : key === 'csrf.token' ? 'token' : fallback),
