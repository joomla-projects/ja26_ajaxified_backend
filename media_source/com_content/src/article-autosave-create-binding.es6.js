...
/** Own the browser-local identity and advisory ownership of one blank Article form. */
export default class ArticleAutosaveCreateBinding {
  constructor({
    context,
    ...
    if (typeof context !== 'string' || context.length === 0 || typeof identityFactory !== 'function') {
      throw new TypeError('The Article create binding configuration is invalid.');
    }
  ...
  resolveFormInstance() {
    ...
    if (typeof identity !== 'string' || !ID_PATTERN.test(identity)) {
      throw new TypeError('The Article form-instance identity is invalid.');
    }
  ...
  bind(identity) {
    if (!isPlainObject(identity) || identity.context !== this.context || !P1_PATTERN.test(identity.target_id)) {
      throw new TypeError('The provisional Article identity is invalid.');
    }
  ...
  subscribeConflict(callback) {
    if (typeof callback !== 'function') {
      throw new TypeError('The Article ownership-conflict callback is invalid.');
    }
