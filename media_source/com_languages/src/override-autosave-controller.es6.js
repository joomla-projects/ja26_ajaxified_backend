...
const validTarget = (target) => {
  ...
  const parts = []; let offset = 3;
  while (offset < target.length) {
    const colon = target.indexOf(':', offset); if (colon < 0) return false;
    const lengthText = target.slice(offset, colon); if (!/^(?:0|[1-9][0-9]{0,2})$/.test(lengthText)) return false;
    const length = Number(lengthText); const start = colon + 1; const part = target.slice(start, start + length);
    if (part.length !== length) return false; parts.push(part); offset = start + length;
    if (offset < target.length) { if (target[offset] !== '|') return false; offset += 1; }
  }
  if (parts.length !== 4 || parts[0] !== 'languages.override' || !['site', 'administrator'].includes(parts[1])
...
const validateConfiguration = (value) => {
  ...
  if (value.context !== 'com_languages.override' || !validTarget(value.targetId) || !Number.isInteger(value.payloadSchemaVersion)
    || typeof value.formId !== 'string' || !value.formId || !isPlainObject(value.fieldIds)
    || Object.keys(value.fieldIds).length !== KEYS.length || !KEYS.every((key) => typeof value.fieldIds[key] === 'string' && value.fieldIds[key])) throw new TypeError('Invalid Language Override Autosave configuration.');
  return Object.freeze({ ...value, fieldIds: Object.freeze({ ...value.fieldIds }) });
};

export default class OverrideAutosaveController extends AutosaveIntegrationController {
  constructor({ documentSource = globalThis.document, optionsReader = defaultOptionsReader, adapterFactory = (options) => new OverrideAutosaveAdapter(options), ...options } = {}) {
    const resolve = () => {
      const config = validateConfiguration(optionsReader(OPTIONS_KEY, null)); if (!config) return null;
      const form = documentSource.getElementById(config.formId); const fields = Object.fromEntries(KEYS.map((key) => [key, documentSource.getElementById(config.fieldIds[key])]));
      if (!form?.isConnected || !KEYS.every((key) => fields[key]?.isConnected && form.contains(fields[key]))) return null;
      return { descriptor: { context: config.context, targetId: config.targetId, payloadSchemaVersion: config.payloadSchemaVersion }, form,
        identityParts: KEYS.map((key) => fields[key]), taskPolicy: TASK_POLICY,
