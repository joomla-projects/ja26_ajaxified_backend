        ...
        foreach ($form->getXml()->xpath('//fields[@name="' . $group . '"]//field') ?: [] as $field) {
            ...
            if (\in_array($type, ['radio', 'list', 'folderlist'], true)) {
                ...
                if ($values !== [] && \count($values) <= AutosaveDynamicSchema::MAXIMUM_ENUM_VALUES) {
                    $descriptors[] = $base + ['kind' => (string) $field['multiple'] === 'true' ? 'strings' : 'enum', 'values' => $values, 'maxItems' => 32];
                }
            } elseif ($type === 'checkbox') {
                $descriptors[] = $base + ['kind' => 'boolean'];
            } elseif (\in_array($type, ['text', 'textarea', 'url', 'number', 'integer', 'int', 'color', 'calendar', 'media'], true)) {
                $limit         = (int) ($field['maxlength'] ?? 0);
                $descriptors[] = $base + ['kind' => 'string', 'maxLength' => $limit > 0 ? min($limit, AutosaveDynamicSchema::MAXIMUM_STRING_SIZE) : AutosaveDynamicSchema::MAXIMUM_STRING_SIZE];
            }
